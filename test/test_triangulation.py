...
def test_traingulation_reprojection():
    P1 = np.array(
        [
            [700.0000, 320.0000, 0.0000, -95.0000],
            [0.0000, 240.0002, -933.3334, 173.3333],
    ...
    x1_reproj = project_3D_to_2D(Xh, P1)
    x2_reproj = project_3D_to_2D(Xh, P2)
    assert np.array_equal(np.round(X, 2), np.round(Xh, 2))
    assert np.linalg.norm(x1_reproj - x1) < 0.05
    assert np.linalg.norm(x2_reproj - x2) < 0.05
